...
The [dataset](https://www.kaggle.com/datasets/amruthayenikonda/dirty-dataset-to-practice-data-cleaning) was obtained from Kaggle user Amrutha Yenikonda. This dataset will be used to practice essential data cleaning and manipulation techniques in Google Sheets, such as removing unwanted features, handling null values, and formatting strings.
...
To improve readability and navigation, I performed the following:
...
---
2.  **Removing Duplicates:** I used the Remove Duplicates function on Google Sheets to search for duplicate records in the data, but no duplicates were found.

<img width="1249" height="502" alt="image" src="https://github.com/user-attachments/assets/b086ddaa-7e14-429b-9353-4c30cbbd674e" />

---
3. **Removing Unnecessary Features:** I removed one unnecessary column (Column K) that contained Wikipedia-style citations used by the original author for referencing, but in our case it has no analytical value.

<img width="357" height="483" alt="image" src="https://github.com/user-attachments/assets/211ea62c-c922-4272-ab23-f7a4ec7c38bb" />

---
4. **Removing Unnecessary Symbols and Characters:** The `tour_title` column (Column G in the raw data) has symbols like `†` and Wikipedia citations like `[4][a]`.
...
---
5. **Changing Strings for Numbers:** The columns `actual_gross`, `adjusted_gross`, and `avg_gross` (Columns D, E, and J) are currently text strings, because they contain `$` symbols and commas.
...
---
7. **Cleaning the Rank Logic:** To finish the cleaning we need to re-number the rank column from 1 to 20 based on the `actual_gross` values, to do this, I typed `1` and `2` in the first two rows, selected them and used the `autofill` feature to fix the rank logic.
